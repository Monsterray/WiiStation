...
void psxBios_Load() { // 0x42
	...
	MTC0(12, psxRegs.CP0.n.SR | 0x404);
}
...
#define burename(slot) { \
	for (i=1; i<16; i++) { \
		...
		ptr[127] = xor; \
		/*SaveMcd(Config.Mcd##slot, Mcd##slot##Data, 128 * i + 0xa, 0x76);*/ \
		v0 = 1; \
		break; \
	} \
	SaveMcdByNum(slot); \
...
#define budelete(slot) { \
	for (i=1; i<16; i++) { \
		...
		if (strcmp(Ra0+5, ptr+0xa)) continue; \
		*ptr = (*ptr & 0xf) | 0xA0; \
		/*SaveMcd(Config.Mcd##slot, Mcd##slot##Data, 128 * i, 1);*/ \
		SaveMcdByNum(slot); \
		mcd##slot##Written = 1; \
